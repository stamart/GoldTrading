...
### `TradeService`
- **`getBuyDay()`**: Finds the day with the lowest price for buying.
...
## Key Considerations
- The application is designed to always select a day to buy and a day to sell, even in cases where there is no potential profit. In scenarios where prices only decline, the application will select the day that results in the smallest possible loss.
...
## Future Improvements
- Implement a more advanced strategy to account for trading fees or restrictions.
- Add user input to allow customization of price range and the number of days.
- Improve the logic for selecting buy and sell days in scenarios with volatile price changes.
